...
func SetupRoutes(router *gin.Engine, linkService *services.LinkService) {
	...
	if ClickEventsChannel == nil {
		...
		// La taille du buffer est configurable via Viper
...
func HealthCheckHandler(c *gin.Context) {
	// Retourne simplement du JSON avec un StatusOK
...
func CreateShortLinkHandler(linkService *services.LinkService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Appelle le LinkService pour créer le nouveau lien.
		...
		// Récupère l'URL de base depuis Viper
...
func RedirectHandler(linkService *services.LinkService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Récupère l'URL longue associée au shortCode depuis le linkService
		...
		// Crée un ClickEvent avec les informations pertinentes
		...
		// Envoie le ClickEvent dans le ClickEventsChannel avec le Multiplexage.
		...
		default:
			// Le channel est plein, on log l'avertissement mais on ne bloque pas
			log.Printf("Warning: ClickEventsChannel is full, dropping click event for %s.", shortCode)
		}

		// Effectue la redirection HTTP 302 (StatusFound) vers l'URL longue
...
func GetLinkStatsHandler(linkService *services.LinkService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Appelle le LinkService pour obtenir le lien et le nombre total de clics
